// js/modules/net-sync.js - ПОЛНОСТЬЮ
            ...
            if (existingChatIndex !== -1) {
                ...
                for (const localMsg of (localChat.messages || [])) {
                    if (!serverMsgIds.has(localMsg.id) && !localMsg.deleted_at) {
                        serverChat.messages.push({
                            ...localMsg,
                            synced: localMsg.synced || false
                        });
                    }
                }
                serverChat.messages.sort((a, b) => {
                    return new Date(a.created_at) - new Date(b.created_at);
                });
                window.chatHistories[topic][existingChatIndex] = serverChat;
                console.log(`🔄 Чат ${chatId} обновлён с сервера (${serverChat.messages.length} сообщений)`);
            } else {
                const existingByName = window.chatHistories[topic].find(c => 
                    c.title === serverChat.title && c.synced && !c.deleted_at
                );
                if (existingByName) {
                    const index = window.chatHistories[topic].indexOf(existingByName);
                    window.chatHistories[topic][index] = serverChat;
                    console.log(`🔄 Чат ${chatId} объединён с существующим (${serverChat.title})`);
                } else {
...
window.handleNewChatClick = function() {
    ...
    if (activeFilter === 'all') {
        ...
        if (typeof window.showTagsCloud === 'function') {
