// js/modules/net-voice.js - ПОЛНОСТЬЮ ИСПРАВЛЕННЫЙ
...
window.toggleVoiceRecording = async function(btn) {
    ...
    const resetVoiceUI = () => {
        ...
        if (userInput) {
            ...
            userInput.placeholder = window.getLangString('placeholder');
        } 
    ...
    try {
        ...
        // ==========================================
        // ИСПРАВЛЕННАЯ НАСТРОЙКА MediaRecorder
        // ==========================================
        ...
        window.mediaRecorder.onstop = async () => {
            ...
            try {
                // ==========================================
                // ИСПРАВЛЕННЫЙ ЗАПРОС С X-Telegram-Init-Data
                // ==========================================
                ...
                if (isExpress) {
                    ...
                    if (typeof window.streamAiResponse === 'function') {
                        ...
                        await window.streamAiResponse(cleanHist, window.currentTopic, userLang, null, activeChat);
                    }
                ...
                if (isExpress) {
                    if (typeof window.hideSkeleton === 'function') window.hideSkeleton();
                    if (typeof window.renderMessageToDOM === 'function') {
                        window.renderMessageToDOM(`⚠️ Сбой сети: ${err.message}`, 'ai-msg');
                    }
                } else if (tg && tg.showAlert) {
