...
// Синхронный HMAC-SHA256 для Edge Runtime (без async/await)
function hmacSha256Sync(key, message) {
    // Простая, но корректная реализация SHA-256
...
export function validateTelegramInitData(initData, botToken) {
    ...
    const secret = hmacSha256Sync('WebAppData', botToken);
    const computedHash = hmacSha256Sync(secret, dataCheckString);
